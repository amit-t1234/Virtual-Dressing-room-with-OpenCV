...
def put_sprite(num):
    global SPRITES, BTNS
    print("This is great: ", num)
    # SPRITES[num - 2] = (1 - SPRITES[num - 2])
    assign = [0, 1, 4, 0, 5, 5, 3]
    SPRITES[0] = (1 - SPRITES[0]) 
    # if SPRITES[num]:
...
def draw_sprite(frame, sprite, x_offset, y_offset):
    ...
    for c in range(3):
        frame[y_offset:y_offset+h, x_offset:x_offset+w, c] = sprite[:,:,c] * (sprite[:,:,c]/255.0) +  frame[y_offset:y_offset+h, x_offset:x_offset+w, c] * (1.0 - sprite[:,:,c]/255.0)
    return frame
...
def detectUpperBody(image):
    cascadePath = "C:\\Users\\amitt\\Desktop\\Folder\\E-Dressing-master\\Project\\data\\haarcascade_upperbody.xml"
    result = image.copy()
...
def cvloop(run_event):
    ...
    # video_capture = cv2.VideoCapture('http://192.168.0.101:4747/mjpegfeed') #read from webcam
    ...
    while run_event.is_set(): 
        ...
        for face in faces: 
            ...
            # Earings
            if SPRITES[4]:
                (x3,y3,w3,h3) = get_face_boundbox(shape, 7) # ear
                apply_sprite(image, image_path,w3,x3-20,y3+25, incl)
                (x3,y3,w3,h3) = get_face_boundbox(shape, 8) # ear
                apply_sprite(image, image_path,w3,x3+20,y3+25, incl)
            ...
            if SPRITES[5]:
                ...
                for obj in findRects:
                    ...
                    # img = cv2.rectangle(imageGray, (obj[0],obj[1]), (obj[0]+obj[2], obj[1]+obj[3]), (0, 255, 0), 2)
                    image = draw_sprite(image, image, obj[0], obj[1])
